...
17. [核心·A1 sweep 焊死] 你做的"抢占率–负载曲线"，为什么说这命名误导？真正的负载信号是什么？
   → 图里两条线两种性质：**P99 TTFT 单调随负载升、拐点在 ~100% KV 预算线**=可信负载信号；**抢占非单调**（峰在 114% 中等超载、131/148% 重超载掉回 ~0）=次级现象，不是负载度量。抢占衡量的是"running 集合有没有贴 KV 天花板顶破"这个局部瞬时事件，跟 KV 压力不直接。量化 KV 压力随负载 → 指 TTFT。数据：results/load-sweep/（3 reps）。

18. [核心·机制] 抢占峰为什么在中等超载(114%)、重超载反而少？
   → 峰不在总负载最高处，在**长请求工作集 ≈ KV 预算**那点（20 长=510k=预算 99%）。轨迹对照 waiting@sat：114% ≈0（全挤进 running、贴天花板→反复顶破→抢占爆发）；148% ≈111（长集 129% 装不下→admission 摁进 waiting、churning running 小→抢占少，压力转 waiting/TTFT）。→ 抢占由稳态 packing 驱动、非总负载。

19. [方法学·面试爱问] 你怎么保证这条曲线可信？遇到过什么坑？
   → burst 下抢占计数**高方差**（同条件 114% 三次 4/12/22），根因 seed 只固定 workload 内容、不固定 asyncio 并发发射时序→不同调度轨迹。单次跑会被离群点骗（首版非单调曲线就栽在拿一个 13 的离群点比其他单次）。修法：每点重复 3 次 + 画 min-max 带 + 用中位数。**这是主动亮方法学 caveat 的加分项**。

20. [顺序对照·证伪] 抢占峰是不是长请求先发导致一开始塞太多？如何证伪？
   → 否。对照 L20x100_interleaved（poisson rate=1000 近 burst 时序 + permutation 打散顺序）：打散后抢占中位 5（vs 长先发 12）**仍不为 0**，waiting@sat 同为 ~1（同 packing）。抢占由稳态 packing（长集撞预算边界）驱动，与顺序无关。顺序只影响 **TTFT 瞬态**（打散后 long P99 反超 short，与长先发翻转）。caveat：n=3 不能排除弱顺序效应，能排除其为驱动。
